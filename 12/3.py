...
if __name__ == "__main__":
    ...
    for index in range(nombre_capteurs):
        ...
        listeTemperatures.append(capteur)

    moyenne_midi = sum(capteur.temp_midi for capteur in listeTemperatures) / len(listeTemperatures)
    print(f"\nTempérature moyenne à midi : {moyenne_midi:.2f}°C")

    listeTemperatures.sort(key=lambda capteur: capteur.ecartMaxTemperaturesCaptees(), reverse=True)

    print("\nCapteurs triés par écart maximal :")
    for capteur in listeTemperatures:
