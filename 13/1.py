...
def enregistrer_etudiants(etudiants, filename):
    with open("etudiants.txt", "w") as f:
        for etudiant in etudiants:
            f.write(f"{etudiant.nom} {etudiant.prenom} ({etudiant.ine}) - {etudiant.email}")
            for note, coef in etudiant.notes_coefficients:
                f.write(f"\n{note}: {coef}")
            f.write("\n")

def recuperer_etudiants(filename):
...
if __name__ == "__main__":
    ...
    while True:
        ...
        print("3. Enregistrer les étudiants dans un fichier")
        print("4. Récupérer les étudiants depuis un fichier")
        print("5. Quitter")
        ...
        if choix == "1":
            ...
            enregistrer_etudiants(etudiants, "etudiants.txt")
        elif choix == "4":
            etudiants = recuperer_etudiants("etudiants.txt")
        elif choix == "5":
            break
        else:
